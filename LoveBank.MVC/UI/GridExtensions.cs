...
namespace LoveBank.MVC.UI
{
    public static class GridExtensions
    {
        ...
        public static IGridColumn<T> CheckBox<T>(this ColumnBuilder<T> column, Func<T, object> keyFunc, Func<T, bool>  ischecked ) where T : class
        {
            return column.Custom(x =>
            {
                var sb = new StringBuilder();
                if (ischecked(x))
                {
                    sb.AppendLine("<input type=\"checkbox\" name=\"{0}\" value=\"{1}\" checked=\"{2}\"/>".FormatWith(
                  "key", keyFunc(x), "checked"));
                }
                else
                {
                    sb.AppendLine("<input type=\"checkbox\" name=\"{0}\" value=\"{1}\"/>".FormatWith(
