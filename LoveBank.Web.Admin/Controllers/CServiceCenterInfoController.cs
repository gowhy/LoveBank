...
{
    [SecurityModule(Name = "网站-社区服务中心")]
    public class CServiceCenterInfoController : BaseController
    {
        //
        // GET: /CServiceCenterInfo/
        const int PageSize = 20;
        [SecurityNode(Name = "首页")]
        public ActionResult Index(CServiceCenterInfoModel model, int? page, int? pageSize)
        {
            ...
            using (LoveBankDBContext db = new LoveBankDBContext())
            {
                ...
                model.CServiceCenterInfoList = list.OrderByDescending(x => x.ID).ToPagedList(pageNumber - 1, size);
                return View(model);
            }
        }
        ...
        {
            var ad = DbProvider.D<CServiceCenterInfo>().FirstOrDefault(x => x.ID == id);
