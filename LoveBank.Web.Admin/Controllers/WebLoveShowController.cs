...
namespace LoveBank.Web.Admin.Controllers
{
   [SecurityModule(Name = "网站-爱心风采展")]
    public class WebLoveShowController : BaseController
    {
        ...
        public ActionResult Index(WebLoveShowModel model, int? page, int? pageSize)
        {
            ...
            using (LoveBankDBContext db = new LoveBankDBContext())
            {
                ...
                model.WebLoveShowList = list.OrderByDescending(x => x.ID).ToPagedList(pageNumber - 1, size);
                return View(model);
            }
        }
        ...
        public ActionResult PostAdd(WebLoveShow model)
        {
            #region 初始化参数
        ...
        public ActionResult Edit(int id)
        {
            using (LoveBankDBContext db = new LoveBankDBContext())
            {
                ...
                var t_wsn = db.T_WebLoveShow;
                WebLoveShow model = t_wsn.Find(id);
                return View(model);
            }
        }
        ...
        public ActionResult PostEdit(WebLoveShow parm)
        {
            using (LoveBankDBContext db = new LoveBankDBContext())
            {

                var t_wsn = db.T_WebLoveShow;
                WebLoveShow model = t_wsn.Find(parm.ID);
        ...
        {
            var ad = DbProvider.D<WebLoveShow>().FirstOrDefault(x => x.ID == id);
