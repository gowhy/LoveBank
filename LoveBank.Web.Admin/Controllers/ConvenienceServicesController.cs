...
{
     [SecurityModule(Name = "网站-便民服务")]
    public class ConvenienceServicesController : BaseController
    {
        //
        // GET: /ConvenienceServices/
        const int PageSize = 20;
        [SecurityNode(Name = "首页")]
        public ActionResult Index(ConvenienceServicesModel model, int? page, int? pageSize)
        {
            ...
            using (LoveBankDBContext db = new LoveBankDBContext())
            {
                ...
                model.ConvenienceServicesList = list.OrderByDescending(x => x.ID).ToPagedList(pageNumber - 1, size);
                return View(model);
            }
        }
        ...
        public ActionResult PostAdd(ConvenienceServices model)
        {
            #region 初始化参数
        ...
        public ActionResult PostEdit(ConvenienceServices parm)
        {
            using (LoveBankDBContext db = new LoveBankDBContext())
            {

                var t_wsn = db.T_ConvenienceServices;
                ConvenienceServices model = t_wsn.Find(parm.ID);
        ...
        {
            var ad = DbProvider.D<ConvenienceServices>().FirstOrDefault(x => x.ID == id);
